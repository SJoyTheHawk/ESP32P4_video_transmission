...
RTSPServer::~RTSPServer() {
  ...
  vSemaphoreDelete(this->sessionsMutex);
}
...
void RTSPServer::deinit() {
  const bool sessionsLocked =
    xSemaphoreTake(this->sessionsMutex, portMAX_DELAY) == pdTRUE;
  const bool maxClientsLocked =
    xSemaphoreTake(this->maxClientsMutex, portMAX_DELAY) == pdTRUE;
  ...
  if (sessionsLocked) {
    for (const auto& sessionPair : this->sessions) {
      ...
      if (session.httpSock >= 0 && session.httpSock != session.sock) {
        close(session.httpSock);
      }
    }
    this->sessions.clear();
  ...
  if (maxClientsLocked) {
    this->maxClients = 1;
    xSemaphoreGive(this->maxClientsMutex);
  }
  this->activeRTSPClients = 0;
...
void RTSPServer::closeSockets() {
  ...
  if (videoRtcpSocket != -1) {
    close(videoRtcpSocket);
    videoRtcpSocket = -1;
  }
...
bool RTSPServer::prepRTSP() {
  ...
  this->videoSequenceNumber = static_cast<uint16_t>(esp_random());
  this->videoTimestamp = esp_random();
  ...
  int reuseAddress = 1;
  setsockopt(this->rtspSocket, SOL_SOCKET, SO_REUSEADDR, &reuseAddress,
             sizeof(reuseAddress));
...
void RTSPServer::rtspTask() {
  ...
  while (true) {
    ...
    if (this->videoRtcpSocket >= 0) {
      FD_SET(this->videoRtcpSocket, &read_fds);
      if (this->videoRtcpSocket > max_sd) {
        max_sd = this->videoRtcpSocket;
      }
    }
    ...
    if (this->videoRtcpSocket >= 0
        && FD_ISSET(this->videoRtcpSocket, &read_fds)) {
      uint8_t rtcpBuffer[256];
      while (recv(this->videoRtcpSocket, rtcpBuffer, sizeof(rtcpBuffer), 0) > 0) {
      }
    }
